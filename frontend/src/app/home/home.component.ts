...
export class HomeComponent implements OnInit {
  ...
  constructor(socketService: SocketService, homeService: HomeService){
  ...
  ngOnInit() {
    ...
    // this.homeService.onGameStarted(this.homeService.localUser()).subscribe((payload: any) => {
    ...
    // this.homeService.onError(this.message).subscribe((error: any) => {
      
    // });

    // this.homeService.onAvailableColors().subscribe((payload: any) => {
  ...
  createRoom() {
    ...
    this.message = '';
    this.homeService.createRoom(this.playerName, this.roomId.trim(),this.socketService.getSocket().id!);
  }
  ...
  joinRoom() {
    ...
    this.message = '';
    this.homeService.joinRoom(this.roomId, this.playerName,this.socketService.getSocket().id!);
  }

  chooseColor(color: string) {
    if (!this.currentRoomId) return;
    this.selectedColor = color;
    this.homeService.selectColor(this.roomId,this.currentRoomId, color);
    this.isPickingColor = false;
  }
  ...
  onError(message: string) {
    this.homeService.onError(message);
    
  }

onAvailableColors(colors: string[]) {
  this.homeService.onAvailableColors(colors);
}

onColorSelected(userId: string,color: string) {
  this.homeService.onColorSelected(userId,color);
}
